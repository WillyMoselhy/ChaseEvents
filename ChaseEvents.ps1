...
#   - Ability to filter on event type (critical/error/warning/information) instead of 
#   - Write to an xlsx file instead of csv to add colors and more sophisticated outputs
#
...
function Show_Help(){
    Write-Host "Chase-Several-Events.ps1 (v1.0)`n" -ForegroundColor Yellow
    ...
    write-host " -Format     : The output format you want to display"
    write-host "               - YMD (Default) : An output of only Year-Month-Day"
    write-host "               - YMDH          : An output of only Year-Month-Day Hourh"
    write-host "               - YMDHM         : An output of only Year-Month-Day Hour:Minute`n"
    write-host "               - YMDHMS         : An output of only Year-Month-Day Hour:Minute:Second`n"
    write-host "               - YMDHMm         : An output of only Year-Month-Day Hour:Minute:Second.Millisecond`n"
    write-host " Notes       : "
...
#   - Filter also par event type (Critical, Error, Warning, Informations)
#   - When exporting to a real Excel file
#       - Add some colors
...
class Event{
    ...
    [string]$C_Date
    [string]$C_EventID
    Event([string]$NodeName, [string]$Date, [string]$ID){
...
# We rely on the naming convention used by the SDP ###_System.evtx
#
...
Get-ChildItem | Where-Object Name -like "*System.evtx" | ForEach-Object { 
    ...
    else
    {
        ...
        $EventsFound | ForEach-Object {
            ...
            $ListOfEvents += [Event]::New($ServerName, $Date, $_.ID)
        }
    }
}
